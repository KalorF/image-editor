...
// oxlint-disable empty-brace-spaces
...
var Potrace = (function () {
  ...
  function loadImageFromGray(buf, width, height) {
    if (info.isReady) {
      clear();
    }
    ...
    var j;
    for (i = 0, j = 0; i < l; i++, j++) {
      bm.data[j] = buf[i] < 128 ? 1 : 0;
    }
    info.isReady = true;
  }

  function loadImageFromGrayAlpha(buf, width, height) {
    if (info.isReady) {
      clear();
    }
    bm = new Bitmap(width, height);
  ...
  function bmToPathlist(bmSource = bm) {
    var bm1 = bmSource.copy();
    ...
    function findPath(point) {
      ...
      path.sign = bmSource.at(point.x, point.y) ? '+' : '-';
    ...
    while ((currentPoint = findNext(currentPoint))) {
      ...
      // const firstPoint = currentPoint.x === 0 && currentPoint.y === 0;
      // const whiteColor = bm1.data[0] === 0 && bm1.data[1] === 0 && bm1.data[2] === 0;
      // const skip = firstPoint && whiteColor
  ...
  function getTransformPath(size, x = 0, y = 0, scaleX = 1, scaleY = 1, pathList) {
    if (pathList) {
      pathlist = pathList;
    }
    function path(curve) {
      function bezier(i) {
        var b =
          'C ' +
          (curve.c[i * 3 + 0].x * size * scaleX + x).toFixed(3) +
          ' ' +
          (curve.c[i * 3 + 0].y * size * scaleY + y).toFixed(3) +
          ',';
        b +=
          (curve.c[i * 3 + 1].x * size * scaleX + x).toFixed(3) +
          ' ' +
          (curve.c[i * 3 + 1].y * size * scaleY + y).toFixed(3) +
          ',';
        b +=
          (curve.c[i * 3 + 2].x * size * scaleX + x).toFixed(3) +
          ' ' +
          (curve.c[i * 3 + 2].y * size * scaleY + y).toFixed(3) +
          ' ';
        return b;
      }
      ...
      var p =
        'M' +
        (curve.c[(n - 1) * 3 + 2].x * size * scaleX + x).toFixed(3) +
        ' ' +
        (curve.c[(n - 1) * 3 + 2].y * size * scaleY + y).toFixed(3) +
        ' ';
      for (i = 0; i < n; i++) {
        if (curve.tag[i] === 'CURVE') {
          p += bezier(i);
  ...
  function getSVG(size, opt_type, bmSource = bm, isGroup = true) {
    function path(curve) {
      function bezier(i) {
        var b =
          'C ' +
    ...
    var w = bmSource.w * size;
    var h = bmSource.h * size;
    var len = pathlist.length;
    var c;
    var i;
    var strokec;
    ...
    var prefix =
      ...
      '<path d="';
    var endfix = '" stroke="' + strokec + '" fill="' + fillc + '"' + fillrule + '/></svg>';

    var svg = prefix;
    for (i = 0; i < len; i++) {
      c = pathlist[i].curve;
      const p = path(c);
      groupPath.push(`${prefix}${p}${endfix}`);
      svg += p;
    }

    svg += endfix;
    return isGroup ? [svg] : groupPath;
  }

  return {
    loadImageFromFile: loadImageFromFile,
    loadImageFromUrl: loadImageFromUrl,
    loadImageFromGray: loadImageFromGray,
    setParameter: setParameter,
    process: process,
    getTransformPath: getTransformPath,
    loadImageFromCanvas,
    loadImageFromGrayAlpha,
    getSVG,
    processPath,
    bmToPathlist,
    img: imgElement,
